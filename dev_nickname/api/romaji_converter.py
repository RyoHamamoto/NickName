...
def _make_romaji_convertor():
    ...
    master = {
        ...
        'ta' :'タ', 'ti':'チ',  'tu' :'ツ', 'tsu' :'ツ','te' :'テ', 'to' :'ト',
    ...
    def __romaji2kana():
        ...
        # 子音が続く時は "ッ" と変換
        rx_xtu = re.compile("([bcdfghjklmpqrstvwxyz])\1")
        # 母音が続く時は "ー" と変換
        rx_a__ = re.compile("([aiueo])\1")
        
        def _romaji2katakana(text):
            result = text.lower()
            result = rx_mba.sub("(?a)ン\1\2", result)
            result = rx_xtu.sub("(?a)ッ\1"  , result)
            result = rx_a__.sub("(?a)\1ー"  , result)
    ...
    def __kana2romaji():
        ...
        rx_xtu = re.compile("(?a)ッ(.)") # 小さい "ッ" は直後の文字を２回に変換
        rx_ltu = re.compile("(?a)ッ$"  ) # 最後の小さい "ッ" は消去(?)
        rx_er  = re.compile("(?a)(.)ー") # "ー"は直前の文字を２回に変換
        rx_n   = re.compile("n(b|p)([aiueo])") # n の後ろが バ行、パ行 なら m に修正
        rx_oo  = re.compile("([aiueo])\1")      # oosaka → osaka
        
        def _kana2romaji(text):
            result = hiragana2katakana(text)
            result = re_kana2roma.sub(lambda x: kana_dict[x.group(0)], result)
            result = rx_xtu.sub("\1\1" , result)
            result = rx_ltu.sub(""     , result)
            result = rx_er.sub ("\1\1" , result)
            result = rx_n.sub  ("m\1\2", result)
            result = rx_oo.sub ("\1"   , result)
            return result
        return _kana2romaji
